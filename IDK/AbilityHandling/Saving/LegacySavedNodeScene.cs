    ...
    public class LegacySavedNodeScene : IRegisterable
    {
        ...
        public static LegacySavedNodeScene Instance(LegacyNodeScene nodescene)
        {
            ...
            for (int i = 0; i < EveryNode.Length; i++)
            {
                ...
                savedNodeScene.blueprints2.Add(EveryNode[i].Blueprint.nodeKey);
                DeveloperLogger.Log($"Saved blueprint for {EveryNode[i].Blueprint.nodeName} ({CurrentID})");
                // Add Position
                savedNodeScene.Positions.Add(CurrentID);
                savedNodeScene.Positions2.Add(EveryNode[i].position);
                DeveloperLogger.Log($"Saved position for {EveryNode[i].Blueprint.nodeName} ({CurrentID})");
                // Add Connections
                if (EveryNode[i].connections != null)
                {
                    ...
                    if (fixedConnections.Count != 0)
                    {
                        ...
                        {
                            DeveloperLogger.Log($"Adding connection {fixedConnections[connectionsIndex].savedNode.Blueprint.nodeName}");
                ...
                DeveloperLogger.Log($"Saved connections for {EveryNode[i].Blueprint.nodeName} ({CurrentID})");
                // Add Fields
                ...
                DeveloperLogger.Log($"Saved fields for {EveryNode[i].Blueprint.nodeName} ({CurrentID})");
        ...
        public LegacyNodeScene SavedNodeSceneToNodeScene()
        {
            ...
            for (int i = 0; i < Positions.Count; i++)
            {
                ...
                UnityEngine.Object.DontDestroyOnLoad(node);

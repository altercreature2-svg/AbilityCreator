...
namespace AC
{
    public class NodeManager : MonoBehaviour
    {
        ...
        public UIManager ui;
        public GameObject hitboxMovement;
        ...
        private GameObject m_abilitySpawner;
        ...
        {
            m_ability = AbilityManager.Instance.CreateAbility(BuildNodeScene(), false);
            m_abilitySpawner = AbilityManager.Instance.CreateIllegalSpawner(m_ability);
        }
        public void DuplicateSelectedNode(NodeComponent node)
        {
            NodeComponent duplicate = AbilityCreator.nodeDatabase[node.nodeBlueprint.nodeKey].Spawn();
            ...
            for (int i = 0; i < tabsButtons.Count; i++)
            {
                ...
                for (int i2 = 0; i2 < tabs.Count * 2; i2++)
                {
                    ...
                    if (i2 <= nodeBlueprints.Where(n => !n.obselete).Count() - 1)
                    {
                        ...
                        button.GetComponentInChildren<TextMeshProUGUI>(true).text = nodeBlueprints[referenceDestroyer2].nodeName;
        ...
        {
            ui.RegisterGenericElement("CANVAS", GameObject.Find("Drawing Board"));
            ui.RegisterGenericElement("HITBOX", GameObject.Find("Hitbox Movement"));
            hitboxMovement = GameObject.Find("Hitbox Movement");
        }
        private IEnumerator Awakeinternal()
        {
            ...
            ResetBlueprints();
            RegisterUiElements();
            StreamedSceneManager.ShowAllObjects(ui.FindElement("CANVAS").go.transform);
            ...
            {
                nodeid = StreamedSceneManager.currentScene.abilityID;
            }
            ...
            if (StreamedSceneManager.currentScene != null)
            {
                GameObject.Find("AbName").GetComponent<TMP_InputField>().text = StreamedSceneManager.currentScene.abilityName;
                GameObject.Find("Description").GetComponent<TMP_InputField>().text = StreamedSceneManager.currentScene.abilityDescription;
                GameObject.Find("Imagename").GetComponent<TMP_InputField>().text = StreamedSceneManager.currentScene.abilityName;
            }
        ...
        private IEnumerator TestUnit(Vector3 position, Vector3 position2)
        {
            ...
            unitBlueprint.objectsToSpawnAsChildren = unitBlueprint.objectsToSpawnAsChildren.AddItem(AbilitySpawner).ToArray();
        ...
        public VirtualNode VirtualizeNode(NodeComponent nodeComponent)
        {
            ...
            {
                connectionTypes.Add(connection.Value.connectionType);
            }
            return virtualNode;
        }
        public VirtualNodeScene BuildNodeScene()
        {
            ...
            DeveloperLogger.Log($"{nodeComponents.Length} nodes to build!");
            List<VirtualNode> nodes = new List<VirtualNode>();
            for (int i = 0; i < nodeComponents.Length; i++)
            {
                if (nodeComponents is null)
                    continue;
                NodeField[] nodeFields = nodeComponents[i].GetComponentsInChildren<NodeField>();
                List<VirtualNodeField> virtualNodeFields = new List<VirtualNodeField>(nodeFields.Length);
                for (int j = 0; j < virtualNodeFields.Count; j++)
                {
                    virtualNodeFields.Add(new VirtualNodeField
                        (
                            i,
                            nodeFields[j].Value
                        )
                ...
                NodeConnector[] nodeConnectors = nodeComponents[i].GetComponentsInChildren<NodeConnector>();
                List<ConnectionType> connectionTypes = new List<ConnectionType>(nodeConnectors.Length);
                for (int j = 0; j < nodeConnectors.Length; j++)
                {
                    ConnectionType connectionType = nodeConnectors[j].connectionType;
                    if (connectionType.second == null)
                        connectionType.second = ConnectionType.VirtualPort.Null;
                    if (connectionType.first == null)
                        connectionType.first = ConnectionType.VirtualPort.Null;
                    connectionTypes.Add(nodeConnectors[j].connectionType);
                }
                VirtualNode virtualNode = new VirtualNode()
                {

                    nodeBlueprint = nodeComponents[i].nodeBlueprint.nodeKey,
                    id = nodeComponents[i].GetNodeInstanceID(),
                    editorPositon = nodeComponents[i].transform.position,
                    fields = virtualNodeFields,
                    connectionTypes = connectionTypes
                };
                nodes.Add(virtualNode);
            }
            virtualNodeScene.savedObjects = nodes.Cast<ISaveable>().ToArray();
        ...
        public void SaveNodeScene()
        {
            ...
            AbilityManager.Instance.UpdateAbility(nodeScene);

            FileManager.WriteAbility(nodeScene.abilityName, Serialize.SaveJson(nodeScene));

...
namespace IDK.NodeScripts
{
    public class CreateFunctionNode : ITriggerNode
    {
        ...
        public override void StartFrame(SavedNode savedNode, Unit unit, List<Node.Connection> connections, string[] fields, NodeRunner nodeRunner)
        {
            ...
            if (functionStorer.actions.Find(n => n.name == fields[0]) == null)
            {
                FunctionStorer.NodeAction nodeAction = new FunctionStorer.NodeAction()
                {
                    name = fields[0],
                };
                nodeAction?.@event?.AddListener(() => Run(savedNode, nodeRunner));
                functionStorer.actions.Add(nodeAction);
            }
            else
            {
                FunctionStorer.NodeAction nodeAction = functionStorer.actions.Find(n => n.name == fields[0]);
                nodeAction.@event.AddListener(() => Run(savedNode, nodeRunner));
            }
        }
